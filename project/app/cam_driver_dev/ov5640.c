...
/* min/typical/max system clock (xvclk) frequencies */
...
struct ov5640_ctrls {
	...
	struct v4l2_ctrl *link_freq;		//Add link freq ctrl
};
...
struct ov5640_dev {
	...
	struct clk *xvclk; /* system clock to OV5640 */
	u32 xclk_freq;
...
//TODO: Calculate actual link frequency, dummy values for now 
...
static int ov5640_set_power(struct ov5640_dev *sensor, bool on)
{
	...
	if (sensor->ep.bus_type == V4L2_MBUS_CSI2_DPHY)
    {
        printk("ov5640 in mipi 2 lane mode");
		ret = ov5640_set_power_mipi(sensor, on);
    }
	else
    {
        printk("ov5640 in dvp mode");
...
static int ov5640_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	sensor->xclk_freq = clk_get_rate(sensor->xvclk);
    ...
    dev_info(dev, "xvclk frequency: %d Hz\n",
        sensor->xclk_freq);
...
MODULE_DEVICE_TABLE(i2c, ov5640_match_id);
